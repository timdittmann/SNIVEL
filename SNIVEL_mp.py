        ...
        for grow in rows:
            ...
            with open(datefile, 'rt') as f:
                ...
                for grow2 in rows2:
                    ...
                    sites_dates_list+=[[site, year, doy, st, nummin]]
    return sites_dates_list

def snivel_process(sites_dates_list):
    site, year, doy, st, nummin = sites_dates_list
    ...
    try:
        ...
        if not os.path.isfile(obsfile):
            obsfile = 'rinex_hr/' + site + doy + '0.' +  year[-2:] + 'd.Z' #rinex file name
        ...
        for i in range (int(tstart),int(tstop)+1):
            ...
            if ((gtime[a1[0]]-gtime[a0[0]])  < 1/sampersec+0.005):
                ...
                clk = "{0:.5f}".format(float(S.x[3])*sampersec)
                ffo.write(str(i)+' '+gpst+' '+nvel+' '+evel+' '+uvel+' '+clk+'\n')
...
def mp_handler():
    ...
    sites_dates_list=make_sites_dates_list(sitefile,datefile)

    #single process orbit file retrieval per doy/year in datesfile
    with open(datefile, 'rt') as f:
            rows2 = (line.split() for line in f)
            for grow2 in rows2:
                    year = grow2[0] #looping over years and days of year to process
    ...
    pool.map(snivel_process, sites_dates_list)
    pool.close()
